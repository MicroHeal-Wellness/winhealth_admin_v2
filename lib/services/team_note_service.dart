...
class TeamNoteService {
  static Future<List<Note>> fetchNotesByPatientId(String patientId) async {
    final response = await BaseService.makeAuthenticatedRequest(
      '${BaseService.BASE_URL}/items/team_notes?filter[patient][_eq]=$patientId&fields=user_created.*,user_updated.*,attachment.*,*&sort[]=-date_created',
      method: 'GET',
    );
  ...
  static Future<bool> addNote(payload) async {
    final response = await BaseService.makeAuthenticatedRequest(
        '${BaseService.BASE_URL}/items/team_notes',
        method: 'POST',
